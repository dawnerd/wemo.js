...
var discover = require('./Discoverer');
...
function WeMo(host, cb) {
  ...
  try {
    ...
    discover(function(WeMos) {
      var wemo;

      for(var i = 0, c = WeMos.length; i < c; i++) {
        wemo = WeMos[i];
        if(wemo.info.device.friendlyName === host) {
          this._host = wemo.location.host;
